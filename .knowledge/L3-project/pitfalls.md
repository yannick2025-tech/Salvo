...
## Lesson 11: trace_id 日志键承载 runID — 用户按页面"TraceID"查日志查不到 (2026-09-21)

### 现象
用户在链路列表看到"TraceID"列（实为 traces 表 DB 主键），拿该值去后端日志检索查不到任何记录；同时 runner 日志中的 `trace_id` 字段实际写入的是运行ID（runID），两层语义错位互相强化误解。

### 根因分析（双向错位）

| # | 问题层 | 影响 |
|---|--------|------|
| **1** | UI 列名"TraceID/ID"指 DB 主键 | 用户拿 DB 主键查日志（日志中根本不存在该值）必然查不到 |
| **2** | runner 4 处（Run/execute/buildDAG/buildScope）+ logger 上下文把 runID 写入 `trace_id` 键 | 日志键名与业务值错位，用户即使误打误撞用 runID 查 trace_id 也要靠猜 |

### 修复方案（UI 统一 + 日志键纠偏 + 智能匹配）

1. **UI 统一**：主键列全站改名"数据KEY"（hover 显示"数据库主键：值"）；业务键列统一"运行ID"（hover 提示"检索后端日志用此 ID"+点击复制）
2. **关联跳转**：场景列表"最后一次运行记录"跳转运行控制页；运行历史"链路"跳转 `/traces?trace_id=<运行ID>` 自动预填过滤条件；运行历史/报告列表场景ID跳转场景列表
3. **日志键纠偏**：logger 新增 `run_id` 上下文键（ContextWithRunID）；runner 全部运行日志从 `trace_id` 改为 `run_id`
4. **智能匹配**：`TraceFilter.TraceID` 改为 `(t.id = ? OR t.run_id = ?)`，过滤框输入数据KEY或运行ID均可命中

### Lessons Learned

#### 1. 同一领域只能有一个名字
- DB 主键、业务键、日志检索键各有语义；任何一处混用都会形成跨系统排查死角

#### 2. 日志键名必须与业务值语义一致
- runID 写入 trace_id 键是历史遗留；修复后日志 `run_id` 与页面"运行ID"一一对应，所见即所查

#### 3. 智能匹配是防御性设计
- 无法阻止用户拿"错"的 ID 查询时，让查询同时接受两种 ID（OR 匹配）兜底，比报错体验更好
