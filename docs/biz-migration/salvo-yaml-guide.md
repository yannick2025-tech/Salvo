...
排查运行问题时，后端日志检索使用**运行ID（run_id）**，而不是页面上的"数据KEY"。两者在各列表页并列展示：

| 名称 | 含义 | 用途 |
|------|------|------|
| 数据KEY | 各数据库表的主键（scenes/runs/traces 等表的 `id`） | 仅标识数据库行；页面 hover 可见"数据库主键" |
| 运行ID（run_id） | 一次场景运行的业务标识（雪花ID），贯穿 run / trace / 后端日志 | **后端日志检索键**；页面显示"运行ID"，点击可复制 |
| 链路ID（chain_id） | 一次 DAG 链路执行的业务标识 | 定位单条 chain 的 span 与日志 |
| 场景ID（scene_id）/ 节点ID（node_id） | 场景 / 节点标识 | 定位场景与节点相关日志 |

要点：

- 运行日志中的 `run_id` 字段即页面上的"运行ID"，用它在日志中可检索到本次运行的全部记录
- 场景列表的"最后一次运行记录"可跳转运行控制页面；运行历史/报告列表的场景ID可跳转场景列表
- 运行历史的"链路"操作跳转链路列表，并自动以运行ID预填过滤条件
- 链路列表的过滤框（"链路/运行ID"）同时支持按数据KEY或运行ID精确匹配
