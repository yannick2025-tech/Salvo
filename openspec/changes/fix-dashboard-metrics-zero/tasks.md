...
- [x] 2.3 验证 Dashboard 的 `buildTimeSeriesWithDB` 查询 `tsStore.QueryByRunID(ctx, run.ID)` 能返回时序数据
- [x] 2.4 验证 `createReport` 中 `r.tsStore.QueryByRunID(reportCtx, r.dbRecordID)` 能返回时序数据
...
- [x] 3.4 验证 Dashboard 的 Pending Queue 图表显示非零的 TaskWait P50/P95 值
...
- [x] 4.3 验证场景停止后 Dashboard 中的延迟值不再随时间变化
...
- [x] 5.5 端到端验证：导出 HTML 报告包含失败节点详情
...
- [x] 6.3 端到端验证：导入 example-v2.yaml，时长模式运行 1 分钟，检查 Dashboard 所有指标正常
- [x] 6.4 端到端验证：导出 HTML 报告，检查时序图表和失败节点详情 section
- [x] 6.5 端到端验证：停止场景后观察 Dashboard 30 秒，确认延迟值不再变化
...
- [x] 8.6 端到端验证：批跑后 Dashboard 节点 P50/P95/P99 显示真实值且停止后不再变化
